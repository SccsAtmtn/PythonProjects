...
from recognize_single import * 
from recognize_batch import * 
...
def recognize1():
    people = recognize_single(filename.get())
    result.set(people)
    #result.set(filename.get())

def recognize2():
    acc = recognize_batch(filedirectory.get())
    accuracy.set(str(acc)+r'%')
    #accuracy.set(str(1))
